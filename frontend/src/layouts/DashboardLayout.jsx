...
const DashboardLayout = ({ role, links, showBackHome = false }) => {
      ...
      <aside className={`w-full md:w-64 bg-brand-900 text-slate-300 md:min-h-screen flex flex-col transition-all duration-300 ease-in-out md:sticky md:top-0 ${isMobileMenuOpen ? 'block' : 'hidden md:flex'}`}>
        ...
        <div className="p-4 border-t border-brand-800 space-y-1">
          {showBackHome && (
            <Link
              to="/"
              className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-slate-400 hover:text-white hover:bg-brand-800 transition-colors"
            >
          ...
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-slate-400 hover:text-red-400 hover:bg-red-900/20 transition-colors">
